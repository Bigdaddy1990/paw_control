...
class PawControlAutomationManager(RestoreEntity):
    ...
    async def async_setup(self) -> None:
        ...
        try:
            ...
            _LOGGER.info(
                "Successfully set up %d automations for %s",
                len(self._automation_registry),
                self._dog_name,
            )
    ...
    async def async_will_remove_from_hass(self) -> None:
        ...
        _LOGGER.info(
            "Cleaned up %d automation listeners for %s",
            len(self._listeners),
            self._dog_name,
        )
    ...
    async def _setup_activity_automations(self) -> None:
        ...
        existing_counters = [
            entity for entity in activity_counters if self.hass.states.get(entity)
        ]
    ...
    async def _setup_health_automations(self) -> None:
        ...
        # Filter for existing entities
        existing_health_entities = [
            entity for entity in health_entities if self.hass.states.get(entity)
        ]
    ...
    async def _setup_emergency_automations(self) -> None:
        ...
        # Filter for existing entities
        existing_emergency_entities = [
            entity for entity in emergency_entities if self.hass.states.get(entity)
        ]
    ...
    async def _setup_visitor_automations(self) -> None:
        ...
        # Filter for existing entities
        existing_visitor_entities = [
            entity for entity in visitor_entities if self.hass.states.get(entity)
        ]
    ...
    async def _handle_health_status_change(self, event: Event) -> None:
        ...
        try:
            ...
            if "health_status" in entity_id:
                await self._handle_health_status_specific_change(
    ...
    async def _handle_daily_summary(self) -> None:
        ...
        try:
            ...
            summary_sensor = self.hass.states.get(
                f"sensor.{self._dog_name}_daily_summary"
            )
    ...
    def _extract_activity_type(self, entity_id: str) -> str:
        ...
        if "walk" in entity_id:
            return "Spaziergang"
        if "play" in entity_id:
            return "Spielzeit"
        if "training" in entity_id:
            return "Training"
        return "Aktivität"
