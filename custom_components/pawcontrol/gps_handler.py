...
class PawControlGPSHandler:
    ...
    def __init__(
        self, hass: HomeAssistant, dog_name: str, config: dict[str, Any]
    ) -> None:
        ...
        self._previous_location: tuple[float, float] | None = None
        self._home_location: tuple[float, float] | None = None
        ...
        self._walk_start_time: datetime | None = None
        ...
        self._gps_entity_id: str | None = None
        self._gps_webhook_id: str | None = None
    ...
    async def async_setup(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "❌ Error setting up GPS handler for %s: %s", self.dog_name, e
            )
            msg = f"GPS handler setup failed: {e}"
            raise GPSError(msg)
        ...
        try:
            ...
            if gps_config_state and gps_config_state.state:
                try:
                    ...
                    self._auto_walk_detection = config_data.get(
                        "auto_walk_detection", False
                    )
                    self._auto_walk_sensitivity = config_data.get(
                        "sensitivity", "medium"
                    )
                except (json.JSONDecodeError, TypeError):
                    _LOGGER.warning(
                        "Invalid GPS config for %s, using defaults", self.dog_name
                    )
            ...
            _LOGGER.info(
                "GPS source configured: %s for %s", self._gps_source_type, self.dog_name
            )

        except Exception as e:
            _LOGGER.exception("Error configuring GPS source: %s", e)
    ...
    async def _setup_device_tracker_source(self) -> None:
        ...
        _LOGGER.info(
            "Device tracker GPS source setup for %s: %s",
            self.dog_name,
            self._gps_entity_id,
        )
    ...
    async def _setup_person_source(self) -> None:
        ...
        _LOGGER.info(
            "Person GPS source setup for %s: %s", self.dog_name, self._gps_entity_id
        )
    ...
    async def _handle_device_tracker_update(self, event: Event) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error handling device tracker update for %s: %s", self.dog_name, e
            )
    ...
    async def _handle_person_update(self, event: Event) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error handling person update for %s: %s", self.dog_name, e
            )
        ...
        try:
            ...
            if "," in location_str:
                try:
                    ...
                    await self.async_update_location(latitude, longitude, 50, "manual")
                except (ValueError, IndexError):
                    _LOGGER.warning(
                        "Invalid manual coordinates for %s: %s",
                        self.dog_name,
                        location_str,
                    )
            ...
            _LOGGER.exception(
                "Error handling manual location update for %s: %s", self.dog_name, e
            )
    ...
    async def async_update_location(
        ...
        try:
            if not validate_coordinates(latitude, longitude):
                msg = f"Invalid coordinates: {latitude}, {longitude}"
            ...
            _LOGGER.debug(
                "GPS updated for %s: %.6f,%.6f (accuracy: %.1fm, source: %s)",
                self.dog_name,
                latitude,
                longitude,
                accuracy,
            ...
            _LOGGER.exception(
                "Error updating GPS location for %s: %s", self.dog_name, e
            )
            msg = f"GPS location update failed: {e}"
            raise GPSError(msg)
    ...
    async def _process_movement(self) -> None:
        ...
        try:
            ...
            if self._last_update and len(self._movement_history) > 0:
                time_diff = (
                    self._last_update - self._movement_history[-1]["timestamp"]
                ).total_seconds()
                if time_diff > 0:
            ...
            if distance_moved >= movement_threshold:
                ...
                self._last_update - self._stationary_since
            ).total_seconds() >= GPS_CONFIG["stationary_time"]:
            ...
            _LOGGER.exception("Error processing movement for %s: %s", self.dog_name, e)
    ...
    async def _update_walk_tracking(self, distance_moved: float) -> None:
        ...
        try:
            ...
            # Update max speed
            self._walk_max_speed = max(self._walk_max_speed, self._speed)
            ...
            if self._walk_start_time:
                walk_duration_hours = (
                    self._last_update - self._walk_start_time
                ).total_seconds() / 3600
            ...
            _LOGGER.exception(
                "Error updating walk tracking for %s: %s", self.dog_name, e
            )
    ...
    async def async_start_walk(self, walk_type: str = "normal") -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error starting walk for %s: %s", self.dog_name, e)
    ...
    async def async_end_walk(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._walk_start_time:
                walk_duration = int(
                    (walk_end_time - self._walk_start_time).total_seconds() / 60
                )
            ...
            calories_burned = self._estimate_calories_burned(
                self._walk_distance, weight
            )

            # Create walk statistics
            walk_stats = {
                "start_time": self._walk_start_time.isoformat()
                if self._walk_start_time
                else None,
                ...
                "accuracy_avg": round(
                    sum(p.get("accuracy", 50) for p in self._walk_route)
                    / len(self._walk_route),
                    0,
                )
                if self._walk_route
                else 50,
            ...
            _LOGGER.info(
                "🏁 Walk ended for %s: %.2fkm in %d minutes",
                self.dog_name,
                self._walk_distance,
                walk_duration,
            )
            ...
            _LOGGER.exception("Error ending walk for %s: %s", self.dog_name, e)
            return {}
    ...
    async def _auto_detect_walk(self) -> None:
        ...
        try:
            ...
            home_distance = calculate_distance(
                self._home_location, self._current_location
            )
            movement_threshold = GPS_CONFIG["walk_detection_distance"]

            # Auto start walk
            if (
                not self._walk_active
                and home_distance > movement_threshold
                and self._is_moving
            ):
                ...
                if self._movement_history:
                    for entry in reversed(self._movement_history[-10:]):
                        if (
                            entry.get("distance_moved", 0)
                            >= GPS_CONFIG["movement_threshold"]
                        ):
            ...
            # Auto end walk
                ...
                if (
                    home_distance <= movement_threshold
                    and self._stationary_since
                    and (now() - self._stationary_since).total_seconds() >= 300
                ):  # 5 minutes stationary
                    walk_stats = await self.async_end_walk()
                    if (
                        walk_stats.get("duration_minutes", 0)
                        >= GPS_CONFIG["min_walk_duration"]
                    ):
                        _LOGGER.info(
                            "Auto-detected walk completed for %s", self.dog_name
                        )

        except Exception as e:
            _LOGGER.exception(
                "Error in auto walk detection for %s: %s", self.dog_name, e
            )
    ...
    async def _setup_default_geofences(self) -> None:
        ...
        try:
            ...
            home_radius = (
                float(home_radius_state.state)
                if home_radius_state
                else GPS_CONFIG["home_zone_radius"]
            )
            ...
            _LOGGER.info(
                "Default geofences setup for %s: home (%.0fm radius)",
                self.dog_name,
                home_radius,
            )

        except Exception as e:
            _LOGGER.exception("Error setting up geofences for %s: %s", self.dog_name, e)
    ...
    async def _check_geofences(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error checking geofences for %s: %s", self.dog_name, e)
    ...
    async def _send_geofence_notification(self, fence_id: str, action: str) -> None:
        ...
        try:
            ...
            _LOGGER.info(
                "Geofence notification sent for %s: %s %s",
                self.dog_name,
                fence_name,
                action,
            )
            ...
            _LOGGER.exception(
                "Error sending geofence notification for %s: %s", self.dog_name, e
            )
    ...
    async def _update_gps_entities(self) -> None:
        ...
        try:
            ...
            status_message = (
                f"Active - {self._gps_source_type} (Accuracy: {self._accuracy:.0f}m)"
            )
            await safe_service_call(
                self.hass,
                "input_text",
            ...
            _LOGGER.exception(
                "Error updating GPS entities for %s: %s", self.dog_name, e
            )
        ...
        try:
            ...
            if self._walk_start_time:
                duration_minutes = int(
                    (now() - self._walk_start_time).total_seconds() / 60
                )
                await safe_service_call(
                    self.hass,
                    "input_number",
            ...
            _LOGGER.exception(
                "Error updating walk entities for %s: %s", self.dog_name, e
            )
    ...
    async def _finalize_walk_entities(self, walk_stats: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error finalizing walk entities for %s: %s", self.dog_name, e
            )
    ...
    async def _load_home_coordinates(self) -> None:
        ...
        try:
            ...
            if home_coords_state and home_coords_state.state:
                try:
                    ...
                    if validate_coordinates(latitude, longitude):
                        ...
                        _LOGGER.info(
                            "Home coordinates loaded for %s: %.6f,%.6f",
                            self.dog_name,
                            latitude,
                            longitude,
                        )
            ...
            await safe_service_call(
                ...
                "input_text",
                "set_value",
                {"entity_id": home_coords_entity, "value": default_coords_str},
            )

            _LOGGER.info(
                "Default home coordinates set for %s: %.6f,%.6f",
                self.dog_name,
                *DEFAULT_HOME_COORDINATES,
            )
            ...
            _LOGGER.exception(
                "Error loading home coordinates for %s: %s", self.dog_name, e
            )
            self._home_location = DEFAULT_HOME_COORDINATES
        ...
        try:
            ...
            if self._last_update:
                ...
                if time_since_update > 600:  # 10 minutes without update
                    ...
                    _LOGGER.warning(
                        "GPS signal lost for %s - %d seconds since last update",
                        self.dog_name,
                        time_since_update,
                    )

        except Exception as e:
            _LOGGER.exception("Error in GPS health check for %s: %s", self.dog_name, e)
    ...
    async def _daily_statistics_reset(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error resetting daily statistics for %s: %s", self.dog_name, e
            )
    ...
    async def _update_gps_statistics(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error updating GPS statistics for %s: %s", self.dog_name, e
            )
            ...
            if len(simplified_route) >= 10:  # At least 10 points
                ...
                if len(routes_json) <= 1000:  # Respect entity limit
                    await safe_service_call(
                        ...
                        "input_text",
                        "set_value",
                        {"entity_id": routes_entity, "value": routes_json},
                    )

        except Exception as e:
            _LOGGER.exception("Error storing walk route for %s: %s", self.dog_name, e)
    ...
    def get_home_location(self) -> tuple[float, float] | None:
        """Get home location."""
    ...
    async def async_set_home_location(self, latitude: float, longitude: float) -> None:
        ...
        _LOGGER.info(
            "Home location updated for %s: %.6f,%.6f",
            self.dog_name,
            latitude,
            longitude,
        )
