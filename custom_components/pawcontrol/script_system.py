...
class PawControlScriptManager:
    ...
    async def async_setup_services(self) -> None:
        ...
        try:
            ...
            _LOGGER.info("Registered %d services for %s", len(services), self._dog_name)

        except Exception as e:
            _LOGGER.exception("Error setting up services for %s: %s", self._dog_name, e)
            raise
    ...
    async def _feed_dog_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error in feed dog service for %s: %s", self._dog_name, e)

    async def _execute_feeding_action(
        self, meal_type: str, portion_size: str, notes: str
    ) -> None:
        ...
        try:
            ...
            await self.hass.services.async_call(
                "input_boolean", "turn_on", {"entity_id": feeding_entity}
            )

            # Increment feeding counter
            counter_entity = f"counter.{self._dog_name}_feeding_{meal_type}_count"
            await self.hass.services.async_call(
                "counter", "increment", {"entity_id": counter_entity}
            )

            # Update last feeding time
            last_feeding_entity = (
                f"input_datetime.{self._dog_name}_last_feeding_{meal_type}"
            )
            await self.hass.services.async_call(
                "input_datetime",
                "set_datetime",
            ...
            _LOGGER.exception("Error executing feeding action: %s", e)
            raise
    ...
    async def _determine_current_meal(self) -> str:
        ...
        try:
            ...
            if hour < 10 and not meals_given.get("morning", False):
                return "morning"
            if 11 <= hour < 15 and not meals_given.get("lunch", False):
                return "lunch"
            if 15 <= hour < 20 and not meals_given.get("evening", False):
                return "evening"
            if not meals_given.get("snack", False):
                return "snack"
            ...
            _LOGGER.exception("Error determining current meal: %s", e)
            return "morning"
    ...
    async def _walk_dog_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error in walk dog service for %s: %s", self._dog_name, e)

    async def _execute_walk_action(
        self, duration: int, distance: str, notes: str, weather: str
    ) -> None:
        """Execute walk action."""
        try:
            ...
            if duration:
                ...
                await self.hass.services.async_call(
                    "input_number",
                    "set_value",
                    {"entity_id": duration_entity, "value": duration},
                )
            ...
            _LOGGER.exception("Error executing walk action: %s", e)
            raise
    ...
    async def _play_with_dog_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.info(
                "Recorded play session for %s: %d minutes", self._dog_name, duration
            )

        except Exception as e:
            _LOGGER.exception(
                "Error in play with dog service for %s: %s", self._dog_name, e
            )

    async def _execute_play_action(
        self, duration: int, play_type: str, intensity: str, notes: str
    ) -> None:
        """Execute play action."""
        try:
            ...
            _LOGGER.exception("Error executing play action: %s", e)
            raise
    ...
    async def _training_session_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_training_action(
                duration, training_type, commands_practiced, success_rate, notes
            )
            ...
            _LOGGER.info(
                "Recorded training session for %s: %d minutes", self._dog_name, duration
            )

        except Exception as e:
            _LOGGER.exception(
                "Error in training session service for %s: %s", self._dog_name, e
            )

    async def _execute_training_action(
        self,
        duration: int,
        training_type: str,
        commands: str,
        ...
        try:
            ...
            _LOGGER.exception("Error executing training action: %s", e)
            raise
    ...
    async def _health_check_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_health_check(
                health_status, weight, temperature, mood, appetite, energy_level, notes
            )
            ...
            _LOGGER.exception(
                "Error in health check service for %s: %s", self._dog_name, e
            )
    ...
    async def _execute_health_check(
        ...
        try:
            ...
            _LOGGER.exception("Error executing health check: %s", e)
            raise
    ...
    async def _medication_given_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_medication_action(
                medication_name, dosage, time_given, notes
            )
            ...
            _LOGGER.info(
                "Recorded medication for %s: %s", self._dog_name, medication_name
            )

        except Exception as e:
            _LOGGER.exception(
                "Error in medication service for %s: %s", self._dog_name, e
            )

    async def _execute_medication_action(
        self, medication: str, dosage: str, time_given: str, notes: str
    ) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error executing medication action: %s", e)
            raise
    ...
    async def _vet_visit_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_vet_visit_action(
                visit_type, diagnosis, treatment, next_appointment, cost, notes
            )
            ...
            _LOGGER.exception(
                "Error in vet visit service for %s: %s", self._dog_name, e
            )

    async def _execute_vet_visit_action(
        self,
        visit_type: str,
        diagnosis: str,
        treatment: str,
        ...
        try:
            ...
            if next_appointment:
                try:
                    ...
                    _LOGGER.warning(
                        "Invalid next appointment date: %s", next_appointment
                    )
            ...
            _LOGGER.exception("Error executing vet visit action: %s", e)
            raise
    ...
    async def _grooming_session_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_grooming_action(
                grooming_type, duration, professional, notes
            )
            ...
            _LOGGER.info(
                "Recorded grooming session for %s: %s", self._dog_name, grooming_type
            )

        except Exception as e:
            _LOGGER.exception(
                "Error in grooming session service for %s: %s", self._dog_name, e
            )

    async def _execute_grooming_action(
        self, grooming_type: str, duration: int, professional: bool, notes: str
    ) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error executing grooming action: %s", e)
            raise
    ...
    async def _emergency_mode_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in emergency mode service for %s: %s", self._dog_name, e
            )
    ...
    async def _activate_emergency_mode(self, reason: str, contact_vet: bool) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error activating emergency mode: %s", e)
            raise
    ...
    async def _deactivate_emergency_mode(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error deactivating emergency mode: %s", e)
            raise
    ...
    async def _visitor_mode_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            await self._execute_visitor_mode_toggle(
                activate, visitor_name, start_time, end_time
            )
            ...
            _LOGGER.exception(
                "Error in visitor mode service for %s: %s", self._dog_name, e
            )

    async def _execute_visitor_mode_toggle(
        self, activate: bool, visitor_name: str, start_time: str, end_time: str
    ) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error executing visitor mode toggle: %s", e)
            raise
    ...
    async def _daily_reset_service(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in daily reset service for %s: %s", self._dog_name, e
            )
    ...
    async def _execute_daily_reset(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error executing daily reset: %s", e)
            raise
    ...
    async def _add_activity_notes(self, activity: str, details: dict[str, str]) -> None:
