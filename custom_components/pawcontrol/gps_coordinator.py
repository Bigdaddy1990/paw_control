...
from .utils import calculate_distance, safe_service_call, validate_coordinates
...
class PawControlDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def async_update_gps_simple(self, data: dict) -> None:
        ...
        if not validate_coordinates(latitude, longitude):
            msg = f"Invalid GPS coordinates: {latitude}, {longitude}"
        ...
        try:
            ...
            _LOGGER.info(
                "GPS updated for %s: %s,%s (accuracy: %sm, source: %s)",
                self.dog_name,
                latitude,
                longitude,
                accuracy,
            ...
            _LOGGER.exception("Error updating GPS for %s: %s", self.dog_name, e)
            msg = f"Failed to update GPS: {e}"
            raise GPSError(msg)
    ...
    async def async_setup_automatic_gps(self, data: dict) -> None:
        ...
        try:
            ...
            await self.hass.services.async_call(
                "input_boolean",
                "turn_on" if auto_start_walk else "turn_off",
            ...
            _LOGGER.info(
                "Automatic GPS tracking setup for %s: source=%s, threshold=%sm",
                self.dog_name,
                gps_source,
                movement_threshold,
            )
            ...
            _LOGGER.exception(
                "Error setting up automatic GPS for %s: %s", self.dog_name, e
            )
            msg = f"Failed to setup automatic GPS: {e}"
            raise GPSError(msg)
    ...
    async def async_start_walk_tracking(self, data: dict) -> None:
        ...
        walk_name = data.get(
            "walk_name", f"Spaziergang {datetime.now().strftime('%H:%M')}"
        )
        ...
        try:
            ...
            _LOGGER.exception(
                "Error starting walk tracking for %s: %s", self.dog_name, e
            )
            msg = f"Failed to start walk tracking: {e}"
            raise GPSError(msg)
    ...
    async def async_end_walk_tracking(self, data: dict) -> None:
        ...
        try:
            ...
            # Get walk start data
            walk_data_entity = self.hass.states.get(
                f"input_text.{self.dog_name}_current_walk_data"
            )
            ...
            avg_speed_kmh = (
                (total_distance_km / (duration_minutes / 60))
            ...
            _LOGGER.exception("Error ending walk tracking for %s: %s", self.dog_name, e)
            msg = f"Failed to end walk tracking: {e}"
            raise GPSError(msg)
    ...
    async def _check_automatic_walk_detection(
        ...
        try:
            ...
            if not self._home_coordinates:
                home_entity = self.hass.states.get(
                    f"input_text.{self.dog_name}_home_location"
                )
            ...
            if distance_from_home > 50 and not await self._is_walk_in_progress():
                auto_detection_entity = self.hass.states.get(
                    f"input_boolean.{self.dog_name}_auto_walk_detection"
                )
                if auto_detection_entity and auto_detection_entity.state == "on":
                    ...
                    _LOGGER.info(
                        "Auto-started walk for %s (%.0fm from home)",
                        self.dog_name,
                        distance_from_home,
                    )
            ...
            # Check if walk should auto-end (< 20m from home for 2+ minutes)
                ...
                if current_distance < 20:  # Still close to home
                    ...
                    _LOGGER.info(
                        "Auto-ended walk for %s (zurück zu Hause)", self.dog_name
                    )

        except Exception as e:
            _LOGGER.exception(
                "Error in automatic walk detection for %s: %s", self.dog_name, e
            )

    async def _update_walk_tracking(
        ...
        try:
            ...
            if len(self._current_walk_points) >= 2:
                recent_distance = (
                    calculate_distance(
                        self._current_walk_points[-2], self._current_walk_points[-1]
                    )
                    / 1000
                )  # km
    ...
    async def _update_final_walk_stats(
        self, duration: int, distance: float, speed: float, calories: int
    ) -> None:
        """Update final walk statistics."""
        try:
            stats_updates = [
            ...
            _LOGGER.exception(
                "Error updating final walk stats for %s: %s", self.dog_name, e
            )
    ...
    async def _increment_walk_counters(self) -> None:
        ...
        try:
            ...
            for counter_id in counters:
                await safe_service_call(
                    self.hass, "counter", "increment", {"entity_id": counter_id}
                )

        except Exception as e:
            _LOGGER.exception(
                "Error incrementing walk counters for %s: %s", self.dog_name, e
            )

    async def _store_walk_in_history(
        self,
        name: str,
        start_time: datetime,
        end_time: datetime,
        ...
        try:
            ...
            history_entity = self.hass.states.get(
                f"input_text.{self.dog_name}_walk_history_today"
            )
            current_history = []
            ...
            _LOGGER.exception("Error storing walk history for %s: %s", self.dog_name, e)
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            _LOGGER.exception(
                "Error updating coordinator data for %s: %s", self.dog_name, e
            )
            return {}
    ...
    async def async_shutdown(self):
        ...
        if await self._is_walk_in_progress():
            await self.async_end_walk_tracking(
                {"walk_rating": 3, "notes": "Automatisch beendet beim Herunterfahren"}
            )
