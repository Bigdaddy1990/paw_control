...
from .const import CONF_DOG_NAME, DOMAIN, ENTITIES, FEEDING_TYPES
from .utils import normalize_dog_name, safe_service_call
...
class PawControlDataUpdateCoordinator(DataUpdateCoordinator):
        ...
        try:
            ...
            return {
                ...
                "health": {
                    ...
                    "last_vet_visit": await self._get_entity_state(
                        "last_vet_visit", None
                    ),
                },
                "activity": {
                    "last_walk": await self._get_entity_state("last_walk", None),
                    "last_feeding": await self._get_entity_state(
                        "last_feeding_morning", None
                    ),
                    "activity_level": await self._get_entity_state(
                ...
                "status": {
                    ...
                    "walk_in_progress": await self._get_boolean_state(
                        "walk_in_progress"
                    ),
                },
    ...
    async def async_feed_dog(self, data: dict[str, Any]) -> None:
        ...
        try:
            data.get("food_type", "Trockenfutter")
            ...
            _LOGGER.info(
                "Feeding recorded for %s: %s (%dg)", self.dog_name, meal_type, amount
            )

        except Exception as e:
            _LOGGER.exception("Error in feed dog service for %s: %s", self.dog_name, e)
    ...
    async def async_start_walk(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in start walk service for %s: %s", self.dog_name, e
            )
    ...
    async def async_end_walk(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error in end walk service for %s: %s", self.dog_name, e)
        ...
        try:
            ...
            if weight:
                await safe_service_call(
                    ...
                    "set_value",
                    {
                        "entity_id": f"input_number.{self.dog_name}_weight",
                        "value": weight,
                    },
                )
            ...
            if temperature:
                await safe_service_call(
                    ...
                    "set_value",
                    {
                        "entity_id": f"input_number.{self.dog_name}_temperature",
                        "value": temperature,
                    },
                )
            ...
            if energy_level:
                await safe_service_call(
                    ...
                    "select_option",
                    {
                        "entity_id": f"input_select.{self.dog_name}_energy_level_category",
                        "option": energy_level,
                    },
                )
            ...
            if health_note:
                await safe_service_call(
                    ...
                    "set_value",
                    {
                        "entity_id": f"input_text.{self.dog_name}_health_notes",
                        "value": health_note,
                    },
                )
            ...
            _LOGGER.exception(
                "Error in log health data service for %s: %s", self.dog_name, e
            )
    ...
    async def async_set_mood(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error in set mood service for %s: %s", self.dog_name, e)
    ...
    async def async_start_training(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in start training service for %s: %s", self.dog_name, e
            )

    async def async_end_training(self, data: dict[str, Any]) -> None:
        """Handle end training service."""
        try:
            data.get("success_rating", 3)
            duration = data.get("duration", 15)
            data.get("learned_commands", "")
            ...
            _LOGGER.exception(
                "Error in end training service for %s: %s", self.dog_name, e
            )
    ...
    async def async_log_medication(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in log medication service for %s: %s", self.dog_name, e
            )
    ...
    async def async_schedule_vet_visit(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            data.get("vet_reason", "Routineuntersuchung")
            ...
            if vet_name:
                await safe_service_call(
                    ...
                    "set_value",
                    {
                        "entity_id": f"input_text.{self.dog_name}_vet_contact",
                        "value": vet_name,
                    },
                )
            ...
            _LOGGER.exception(
                "Error in schedule vet visit service for %s: %s", self.dog_name, e
            )
    ...
    async def async_start_playtime(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in start playtime service for %s: %s", self.dog_name, e
            )
    ...
    async def async_end_playtime(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            data.get("fun_rating", 5)
            data.get("energy_afterwards", "Entspannt")
            ...
            _LOGGER.exception(
                "Error in end playtime service for %s: %s", self.dog_name, e
            )
    ...
    async def async_reset_all_data(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error in reset all data service for %s: %s", self.dog_name, e
            )
    ...
    async def _add_to_daily_amount(self, entity_suffix: str, amount: float) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Error adding to daily amount %s: %s", entity_suffix, e)
...
async def async_create_helpers(
    hass: HomeAssistant, dog_name: str, config: dict[str, Any]
) -> None:
    ...
    try:
        ...
        _LOGGER.exception("Error creating helper entities for %s: %s", dog_name, e)
        raise


async def _create_input_boolean_entities(hass: HomeAssistant, dog_name: str) -> None:
    """Create input_boolean entities."""
    for _entity_suffix, entity_config in ENTITIES.get("input_boolean", {}).items():
        await safe_service_call(
            hass,
            "input_boolean",
            "create",
...
async def _create_input_number_entities(hass: HomeAssistant, dog_name: str) -> None:
    ...
    for _entity_suffix, entity_config in ENTITIES.get("input_number", {}).items():
        service_data = {
            "name": f"{dog_name.title()} {entity_config['name']}",
...
async def _create_input_text_entities(hass: HomeAssistant, dog_name: str) -> None:
    ...
    for _entity_suffix, entity_config in ENTITIES.get("input_text", {}).items():
        service_data = {
            "name": f"{dog_name.title()} {entity_config['name']}",
...
async def _create_input_datetime_entities(hass: HomeAssistant, dog_name: str) -> None:
    ...
    for _entity_suffix, entity_config in ENTITIES.get("input_datetime", {}).items():
        service_data = {
            "name": f"{dog_name.title()} {entity_config['name']}",
...
async def _create_counter_entities(hass: HomeAssistant, dog_name: str) -> None:
    ...
    for _entity_suffix, entity_config in ENTITIES.get("counter", {}).items():
        service_data = {
            "name": f"{dog_name.title()} {entity_config['name']}",
...
async def _create_input_select_entities(hass: HomeAssistant, dog_name: str) -> None:
    ...
    for _entity_suffix, entity_config in ENTITIES.get("input_select", {}).items():
        service_data = {
            "name": f"{dog_name.title()} {entity_config['name']}",
...
async def get_dog_status_summary(hass: HomeAssistant, dog_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        _LOGGER.exception("Error getting status summary for %s: %s", dog_name, e)
        return {
            "dog_name": dog_name,
            "error": str(e),
            "last_updated": datetime.now().isoformat(),
...
async def send_dog_notification(
    hass: HomeAssistant, dog_name: str, title: str, message: str
) -> None:
    ...
    try:
        ...
        _LOGGER.exception("Error sending notification for %s: %s", dog_name, e)
    ...
    try:
        ...
        for domain in domains_to_clean:
            ...
            for entity_id in domain_entities:
                try:
                    await hass.services.async_call(
                        domain, "remove", {"entity_id": entity_id}, blocking=True
                    )
                    await asyncio.sleep(0.1)
                except Exception as e:
        ...
        _LOGGER.exception("Error during entity cleanup for %s: %s", dog_name, e)
...
def parse_datetime(dt_str: str) -> datetime | None:
    ...
    try:
        ...
        _LOGGER.exception("Failed to parse datetime '%s': %s", dt_str, e)
        return None
...
def generate_automation(
    trigger: dict, action: dict, condition: dict | None = None
) -> dict:
    automation = {"trigger": [trigger], "action": [action]}
    if condition:
...
def build_time_trigger(at_time: str) -> dict:
    return {"platform": "time", "at": at_time}


def build_state_trigger(
    entity_id: str, from_state: str | None = None, to_state: str | None = None
) -> dict:
    trig = {"platform": "state", "entity_id": entity_id}
...
def send_mobile_notification(hass, message: str, target_device: str | None = None):
