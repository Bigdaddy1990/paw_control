...
async def async_log_activity(
    hass: HomeAssistant, dog_name: str, activity_type: str, notes: str | None = None
) -> None:
    ...
    try:
        ...
        if activity_type in ["walk", "feeding", "outside", "poop"]:
            ...
            if hass.states.get(datetime_entity):
                await hass.services.async_call(
                    "input_datetime",
                    "set_datetime",
                    {"entity_id": datetime_entity, "datetime": timestamp.isoformat()},
                )
        ...
        if hass.states.get(last_activity_entity):
            await hass.services.async_call(
                "input_text",
                "set_value",
                {"entity_id": last_activity_entity, "value": last_activity_text},
            )
        ...
        if hass.states.get(activity_notes_entity):
            ...
            await hass.services.async_call(
                "input_text",
                "set_value",
                {"entity_id": activity_notes_entity, "value": new_notes},
            )
...
async def async_log_feeding(
    hass: HomeAssistant, dog_name: str, meal_type: str, amount: int | None = None
) -> None:
...
async def async_log_health_event(
    hass: HomeAssistant, dog_name: str, event_type: str, details: str | None = None
) -> None:
    ...
    await async_log_activity(
        ...
        dog_name,
        "health_event",
        f"{event_label}: {details}" if details else event_label,
    )
