...
async def send_push_notification(
    ...
    for target in recipients:
        try:
            ...
            _LOGGER.exception("Failed to send notification to %s: %s", target, e)


async def send_feeding_reminder(
    hass: HomeAssistant, dog_name: str, meal_type: str
) -> None:
...
async def send_walk_reminder(hass: HomeAssistant, dog_name: str) -> None:
    ...
    actions = [
        {"action": f"WALK_START_{dog_name.upper()}", "title": "Gassi starten 🚶"},
        {"action": f"WALK_LATER_{dog_name.upper()}", "title": "Später 🕐"},
    ]
...
async def send_health_alert(
    hass: HomeAssistant, dog_name: str, alert_type: str
) -> None:
    ...
    actions = [{"action": f"HEALTH_CHECK_{dog_name.upper()}", "title": "Überprüft ✅"}]

    if alert_type == "emergency":
        actions.append(
            {"action": f"CALL_VET_{dog_name.upper()}", "title": "Tierarzt anrufen 📞"}
        )
...
async def send_test_notification(hass: HomeAssistant, dog_name: str) -> None:
    ...
    actions = [
        {"action": f"TEST_OK_{dog_name.upper()}", "title": "Test erfolgreich ✅"}
    ]
